...
def direct_run_progress_notification(user_id: str):
    """直接执行部门成员进度更新，不依赖调度器实例"""
    try:
        # 直接创建ProgressUpdateScheduler实例并使用其核心方法
        from app.modules.sched.progress_update_sched import ProgressUpdateScheduler
        from flask import current_app
        
        app = current_app._get_current_object()
        # 创建临时实例，但不启动调度器
        temp_scheduler = ProgressUpdateScheduler(app)
        ...
        if success:
            ...
            return jsonify({
                "code": Response.r.ERR_INTERNAL,
                "message": "执行部门成员进度更新失败",
                "data": None,
                "status": "ERROR"
            }), 500
            
    except Exception as e:
        logging.error(f"直接执行部门成员进度更新失败: {str(e)}")
        return jsonify({
            "code": Response.r.ERR_INTERNAL,
            "message": f"直接执行部门成员进度更新失败: {str(e)}",
            "data": None,
            "status": "ERROR"
        }), 500
...
def direct_force_progress_check(user_id: str):
    """强制执行部门成员进度更新（不受时间限制），仅供管理员和组长使用"""
    try:
        # 直接创建ProgressUpdateScheduler实例并使用其核心方法
        from app.modules.sched.progress_update_sched import ProgressUpdateScheduler
        from flask import current_app
        ...
        if success:
            ...
            return jsonify({
                "code": Response.r.ERR_INTERNAL,
                "message": "强制执行部门成员进度更新失败",
                "data": None,
                "status": "ERROR"
            }), 500
            
    except Exception as e:
        logging.error(f"强制执行部门成员进度更新失败: {str(e)}")
        return jsonify({
            "code": Response.r.ERR_INTERNAL,
            "message": f"强制执行部门成员进度更新失败: {str(e)}",
            "data": None,
            "status": "ERROR"
        }), 500
    ...
    try:
        ...
        try:
            if request.is_json:
                ...
                # 如果不是JSON请求，使用空字典
                logging.warning("请求Content-Type不是application/json，使用默认参数")
                data = {}
        except Exception as parse_error:
            logging.error(f"解析请求数据出错: {str(parse_error)}")
            data = {}
        ...
        # 直接执行批量进度更新逻辑
        with app.app_context():
            result = temp_scheduler.run_batch_update_now(department_id=department_id)
    ...
    try:
        ...
        with app.app_context():
            try:
                ...
                logging.error(f"批量更新过程中出错: {str(e)}")
                return jsonify({
                    "code": Response.r.ERR_INTERNAL,
                    "message": f"批量更新过程中出错: {str(e)}",
                    "data": None,
                    "status": "ERROR"
                }), 500
