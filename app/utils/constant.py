...
class LLMStructure:
    ...
    class DailyReport(BaseModel):
        ...
        excess: Review
        extra: Review
        efficiency: Review  # 添加效率评分
        innovation: Review  # 添加创新评分
        total: Review
...
class LocalPath:
    ...
    HONOR_PICTURE = os.path.join(ROOT_PATH, "public", "honors")  # 荣誉证书图片存储路径
...
class UrlTemplate:
    ...
    HONOR_PICTURE = lambda filename: f"/public/honors/{filename}"  # 荣誉证书图片URL模板
...
4. 学习效率（不参与评分，但需要给出分数，满分100）
学习效率：评估用户的学习效率，包括学习时间、学习质量、学习效果等方面。如果用户的学习效率高，可以额外增加分数。
5. 创新应用（不参与评分，但需要给出分数，满分100）
创新应用：评估用户在学习或完成任务过程中是否展现出了创新思维、独立思考、问题解决能力等方面。如果用户在学习或完成任务过程中展现出了创新思维，可以额外增加分数。
反馈生成：
...
4. 学习效率正面反馈：如果用户在学习或完成任务过程中展现出了高效率，生成积极反馈。例如：
"你今天的学习效率很高，今天的评分为：{评分}。"
5. 创新应用正面反馈：如果用户在学习或完成任务过程中展现出了创新思维，生成积极反馈。例如：
"你今天展现出了很好的创新思维，今天的评分为：{评分}。"
图片说明
...
请将所有评分内容以JSON格式返回，JSON结构如下：
{
    ...
    "excess": {
        "review": "与任务相关的额外内容完成情况",
        "score": 超额内容评分
    },
    "extra": {
        "review": "其他非任务相关内容的完成情况",
        "score": 额外内容评分
    },
    "efficiency": {
        "review": "通过学习时间与学习内容量的比值来评估学习效率情况",
        "score": 学习效率评分  # 不计入总分
    },
    "innovation": {
        "review": "通过学习内容与任务完成情况的比值来评估创新应用情况",
        "score": 创新应用评分  # 不计入总分
    },
    "total": {
        "review": "总评价和鼓励语句",
        "score": 总分数  # 只包含 basic + excess + extra
    }
}
"""
    )
...
{ "base": 基本评分, "excess": 超额评分, "extra": 额外评分,"efficiency":效率评分,"innovation":应用评分 }
文本：
